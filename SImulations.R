...
#Vector containing statistics for the algorithms, in order Corrected oriented edges, wrongly oriented edges, missing edges, extra edges and False discovery rate
...
T_p<-T_pt<-T_tp<-0
    ...
    end_time<-Sys.time()
    S_p<-diff_stat(Ip,Itestdirected)                #Statistic vector
    T_p<-time+end_time-start_time                   #Pairwise Runtime
    ...
    end_time<-Sys.time()
    S_pt<-diff_stat(Itp,Itestdirected)             #Statistic vector
    T_pt<-time+end_time-start_time                 #PT runtime
    
    #TP
    start_time<-Sys.time()
    Olistpt<-new_tmomcor_third_fourth_cum(x,C,S,E_n,thres)
    Ipt<-edgelist_toadjmatrix(Olistpt)             #Adjacency matrix
    end_time<-Sys.time() 
    S_tp<-diff_stat(Ipt,Itestdirected)             #Statistic vector
    T_tp<-time+end_time-start_time                 #TP runtime
